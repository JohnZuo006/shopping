...
function chooseProvince(th){//通过方法的调用来实现省 市之间的二级联动，th是我们设置的一个参数，方便下面进行使用，可以理解为province的一个元素（名字）
	...
	for(var n=0; n<provinceList.length; n++){//通过循环遍历列表数组
		...
		if (provice.name == provinceName){//开始建立连接，通过if判断，前提是要满足if中的条件
			...
			cityTag.innerHTML="";//这个步骤很重要，分析语句，是吧里面的内容变为空，意思就是当你选择一个省时，后面会出现城市的数据，当你选择另外的省时会出现相应的市的数据，如果不删除就会同时显示在一个选择框中，就不会是我们想要的结果
	...
	//alert(provinceName);
	document.getElementById("pp").value=provinceName;
}
...
function chooseCity(ci){
	...
	document.getElementById("cc").value=cityName;
}
function chooseArea(ai){
	//var areaTag = document.getElementById("area");
	var index = ai.selectedIndex;
	var areaName = areaArray[index];
	document.getElementById("aa").value=areaName;
}
