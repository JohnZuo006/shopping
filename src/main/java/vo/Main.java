...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(main.test_trade_precreate("12020062217383361795"));
    }
    ...
    // 测试当面付2.0查询订单
    public String test_trade_query(String orderNo) {
    	String resp="";
    ...
    public String test_trade_precreate(Order order) {
        ...
        // 商品明细列表，需填写购买商品详细信息，
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        for(OrderDetails d:order.getOrderDetail())
        {
        	// 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
            GoodsDetail goods1 = GoodsDetail.newInstance(Integer.toString(d.getProductId()),
            		d.getProductName(), d.getCurrentUnitPrice().multiply(new BigDecimal(100)).longValue(),d.getQuantity());
            // 创建好一个商品后添加至商品明细列表
            goodsDetailList.add(goods1);
        }
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
            ...
            .setNotifyUrl("http://ser.zuoxinlei.top:38000/shopping/Pay?type=callback")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("D:/www/tmp/qr-%s.png",
                    response.getOutTradeNo());
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                resp=response.getOutTradeNo();
                break;
